#Este esta mal, lo guardo porque no habia entendido el problema, esto es mucho mas complicado. Solo habia que sacar las veces que el valor es 0
...
Get-Content '.\inputprueba' | ForEach-Object {
    $valorAntes = $valor
    ...
    if ($numeroString.Length -ge 3) {
        #Write-Host $numeroString "es mas de 2"
        $sumarVueltas = $numeroString.Substring(0, ($numeroString.Length - 2))
        #Write-Host "voy a sumar" $sumarVueltas a $vueltas
        $vueltas = $vueltas + $sumarVueltas
    ...
    elseif ($_ -match "R") {
        ...
        while (($valor + $numeroString) -gt 99) {
            $valor = $valor - 100
            $vueltas++
        }
        $valor = $valor + $numeroString
